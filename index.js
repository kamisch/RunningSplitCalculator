$(document).ready(function () {
    ...
    var outputData = function (tps) {
        $('#output').empty();
        let unitMessage = "<h4>Unit: {} | below is your splits | format H:mm:ss </h4>".format(disUnit);
        let hundredMeters = "<h4>100 Meters: {} </h4>".format(timeConversion(100,tps));
        let twoHundredMeters = "<h4>200 Meters: {} </h4>".format(timeConversion(200,tps));
        let fourHundredMeters = "<h4>400 Meters: {} </h4>".format(timeConversion(400,tps));
        let eightHundredMeters = "<h4>800 Meters: {} </h4>".format(timeConversion(800,tps));
        let oneMile = "<h4>1600 Meters/1 Mile: {} </h4>".format(timeConversion(1600,tps));
        ...
        let halfMarathon = "<h4>21.1 km/13.1 Miles: {} </h4>".format(timeConversion(21097.5,tps));
        let Marathon = "<h4>42.2 km/26.2 Miles: {} </h4>".format(timeConversion(42195,tps));
    ...
    var inputOutOfBound = function () {
        ...
        if (Number.isNaN(hourVal) || Number.isNaN(minVal) || Number.isNaN(secVal) || Number.isNaN(disVal)) {
